import type { OGSMData, Goal, Strategy, Measure, ActionPlan, PlanItem, KPI, Status } from '../types/ogsm';
...
export function extractKPIs(text: string): KPI[] {
  ...
  for (const line of lines) {
    ...
    const targetMatch = line.match(/目標[：:]?\s*(?:觸及\s*)?(\d+(?:[,，]\d+)?)\s*(人|名|筆|組|次)?/);
    const actualMatch = line.match(/(?:實際|已)[^，,。\n]{0,10}?(\d+(?:[,，]\d+)?)\s*(人|名|筆|組)/);

    kpis.push({
      id: genId('kpi'),
      label: line.trim().substring(0, 40).replace(/\s+/g, ' '),
      target: targetMatch ? parseFloat(targetMatch[1].replace(/[,，]/g, '')) : null,
      actual: actualMatch ? parseFloat(actualMatch[1].replace(/[,，]/g, '')) : null,
...
function parseActionPlans(q1Text: string, q2Text: string): ActionPlan[] {
  ...
  function parseQ(text: string, quarter: 'Q1' | 'Q2') {
    ...
    if (sections.length === 0) {
      plans.push({ id: genId('plan'), quarter, title: `${quarter} 行動計畫`, items: parseItems(text) });
      return;
    }

    for (let i = 0; i < sections.length; i++) {
      const content = text.substring(sections[i].start, i + 1 < sections.length ? sections[i + 1].start - sections[i + 1].title.length - 2 : text.length);
      plans.push({ id: genId('plan'), quarter, title: sections[i].title, items: parseItems(content) });
    }
  }
  ...
  function parseItems(text: string): PlanItem[] {
    return text.split('\n')
      ...
      .map(line => {
        ...
        const description = dateMatch ? line.substring(dateMatch[0].length).trim() : line;
        const completed = /完成|結案|啟動/.test(description) && /^\d+\/\d+/.test(line);
...
export function parseOGSM(csvText: string): OGSMData {
  ...
  const addStrategy = (bText: string, cText: string, dText: string, eText: string, fText: string, gText: string) => {
  ...
  for (const row of dataRows) {
    ...
    const aT = a.trim(), bT = b.trim(), cT = c.trim(), dT = d.trim(), eT = e.trim(), fT = f.trim(), gT = g.trim();

    if (aT && /^G\d/.test(aT)) {
      const label = aT.match(/^(G\d+)/)?.[1] ?? 'G?';
      const title = aT.match(/^G\d+[：:]\s*([^\n]+)/)?.[1]?.trim() ?? aT.substring(0, 50);
      currentGoal = { id: genId('goal'), label, title, fullText: aT, strategies: [], completionRate: 0 };
      ...
      ref.currentStrategy.measures.push({ id: genId('msr'), rawText: cT, kpis });
  ...
  return { objectives: { orgO, deptO }, goals, period: '2026 H1', importedAt: new Date().toISOString(), overallRate };
}
