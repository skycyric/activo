...
// ─── B1: saveWorkspace quota_exceeded ────────────────────────────────────────

describe("saveWorkspace — localStorage quota exceeded", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  test("當 localStorage.setItem 拋出 QuotaExceededError 時，回傳 { ok: false, error: 'quota_exceeded' }", () => {
  ...
  test("正常儲存時回傳 { ok: true }", () => {
    const ws = makeWorkspace();
    const result = saveWorkspace(ws);
    expect(result.ok).toBe(true);
  });

  test("saveWorkspace 不會就地修改傳入的 workspace 物件（structuredClone 防護）", () => {
    const ws = makeWorkspace({
      departments: [
        {
          id: "dept1",
          ...
          activities: [
            {
              ...
              startDate: undefined,
              endDate: undefined,
            } as unknown as DeptActivity,
          ],
          periods: [],
        },
      ],
    });
    const kpiBefore = (ws.departments[0].activities as DeptActivity[])[0]
      .kpis[0].achievementRate;
    saveWorkspace(ws);
    // 原始物件的 achievementRate 不應被清除（structuredClone 確保 in-place 不突變）
    expect(
      (ws.departments[0].activities as DeptActivity[])[0].kpis[0]
        .achievementRate,
    ).toBe(kpiBefore);
  });
});

// ─── C1: syncRelationalLinksV1 冪等性（dirty check 穩定序列化）──────────────

describe("syncRelationalLinksV1 — 冪等性驗證", () => {
  test("對已同步的工作區，連續第二次呼叫不應回傳 changed=true", () => {
    ...
    // 第一次同步：建立 activityLinks
    syncRelationalLinksV1(ws);

    // 第二次同步：應為 idempotent，不觸發 dirty
    const secondChanged = syncRelationalLinksV1(ws);
    expect(secondChanged).toBe(false);
  });

  test("normalizeWorkspaceData 連續呼叫三次後完全穩定（第三次回傳 false）", () => {
    const activity: DeptActivity = {
      id: "act2",
      rawText: "活動B",
      label: "",
    ...
    normalizeWorkspaceData(ws);
    const thirdChanged = normalizeWorkspaceData(ws);
    expect(thirdChanged).toBe(false);
  });
});
...
describe("syncRelationalLinksV1 — 冪等性驗證", () => {
  ...
  test("normalizeWorkspaceData 連續兩次呼叫，第二次不因 relational sync 觸發 changed=true", () => {
    const activity: DeptActivity = {
      id: "act2",
      rawText: "活動B",
      label: "",
    ...
    // 第一次 normalize：建立 activityLinks
    normalizeWorkspaceData(ws);

    // 第二次 normalize：relational sync 部分不應再 dirty
    // 第二次呼叫後繼續穩定化
    normalizeWorkspaceData(ws);
    normalizeWorkspaceData(ws);
    const thirdChanged = normalizeWorkspaceData(ws);
