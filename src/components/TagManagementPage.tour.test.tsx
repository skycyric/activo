...
describe("TagManagementPage guided tour", () => {
  ...
  test("delete dialog should close when tour leaves the delete steps", async () => {
    mockedTour.isActive = true;
    mockedTour.step = {
      id: "tags-delete-dialog",
      page: "tags",
    ...
    // Simulate tour ending (step changes away from delete dialog steps)
    act(() => {
      mockedTour.step = {
        id: "tags-list",
        page: "tags",
    ...
    await waitFor(() => {
      expect(screen.queryByLabelText("刪除標籤設定")).not.toBeInTheDocument();
    });
  });
});
